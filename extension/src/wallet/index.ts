...
export class Server {
  ...
  async callClientFunction(
    ...
    console.log('callClientFunction', functionName, args)
    if (!this.client) {
  ...
  async init() {
    ...
    chrome.runtime.onMessage.addListener((message, sender, respond) => {
      ...
      if (guard.isCallRequest(message)) {
        this.callClientFunction(sender, message.function, ...message.arguments).then(respond)
        return true
      } else if (guard.isSetWalletRequest(message)) this.setWallet(message.wallet)
      else if (guard.isGetWalletRequest(message)) respond(this.wallet)
      else console.warn('Unknown message', message)
