...
export class WalletPicker extends LitElement {
  ...
  render = () => html`
    <form>
      <label
        class="button button--standard button--primary button--medium button--has-label"
        for="wallet"
        ><span class="button__label">Set wallet…</span></label
      >
...
export class ConfirmButton extends LitElement {
  ...
  render = () => html`
    <sl-button @click=${this.confirmSomething} variant="primary">
      Confirm something ${this.confirmed === true ? '✓' : this.confirmed === false ? '✗' : ''}
    </sl-button>
  `
...
export class Sidebar extends LitElement {
  ...
  render() {
    return html`
      ...
      <linera-wallet-picker
        .onChange=${(wallet: string) => this.onWalletChange(wallet)}
      ></linera-wallet-picker>
